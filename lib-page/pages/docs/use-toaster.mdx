...
You can import only the core of the library with `every-notify/headless`. It won't include any styles, dependencies or custom components.

```jsx
import { useToaster } from 'every-notify/headless';
```

Be aware: [every-notify 2.0](/docs/version-2) adds support for **custom render functions**, an easier method to render custom notification components.
...
```jsx
import toast, { useToaster } from every-notify/headless';
